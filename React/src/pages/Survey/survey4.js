...
function Survey({ navigation }) {
  ...
  const data = async (imp, sat, fb) => {
    try {
      await AsyncStorage.setItem("imp4", JSON.stringify(imp));
      await AsyncStorage.setItem("sat4", JSON.stringify(sat));
      await AsyncStorage.setItem("fb4", JSON.stringify(fb));
    } catch (error) {
      console.log(error);
    }
  };
  ...
  return (
    <View style={Styles.container}>
      ...
      <View style={Styles.survey}>
        <Text style={Styles.question}>4. Cumprimento do horario das aulas</Text>
      ...
      <View style={Styles.buttons}>
        ...
        ((satisfacao === "regular" || satisfacao === "ruim") &&
          ...
          <TouchableOpacity
            ...
            onPress={() => (navigation.navigate("Survey5"), data(importancia, satisfacao, feedback))}
          >
            <Icon name="ios-arrow-forward" style={Styles.seta}></Icon>
          </TouchableOpacity>
        )}
