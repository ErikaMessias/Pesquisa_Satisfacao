...
function Survey({ navigation }) {
  ...
  const data = async (imp, sat, fb) => {
    try {
      await AsyncStorage.setItem("imp6", JSON.stringify(imp));
      await AsyncStorage.setItem("sat6", JSON.stringify(sat));
      await AsyncStorage.setItem("fb6", JSON.stringify(fb));
    } catch (error) {
      console.log(error);
    }
  };
      ...
      <View style={Styles.buttons}>
        ...
        ((satisfacao === "regular" || satisfacao === "ruim") &&
          ...
          <TouchableOpacity
            ...
            onPress={() => (navigation.navigate("Survey7"), data(importancia, satisfacao, feedback))}
          >
            <Icon name="ios-arrow-forward" style={Styles.seta}></Icon>
          </TouchableOpacity>
        )}
